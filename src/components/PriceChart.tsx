...
const PriceChart: React.FC<PriceChartProps> = memo(({ data }) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="custom-tooltip">
        <p className="tooltip-time">{`Time: ${String(label)}`}</p>
        {payload.map((entry: any, index: number) => (
          <p
            key={index}
            style={{ color: String(entry.color), margin: "2px 0" }}
          >
            {`${String(entry.name)}: $${entry.value?.toFixed(2) ?? "N/A"}`}
          </p>
        ))}
      </div>
    );
  ...
  const yAxisDomain = useMemo(() => {
    ...
    for (const point of data) {
      ...
      if (point.coinbase && visibleLines.coinbase)
        allPrices.push(point.coinbase);
      if (point.pyth && visibleLines.pyth) allPrices.push(point.pyth);
      if (point.pythlazer && visibleLines.pythlazer)
        allPrices.push(point.pythlazer);
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              alignItems: "center",
              cursor: "pointer",
              opacity: visibleLines[item.dataKey as keyof typeof visibleLines]
                ? 1
                : 0.4,
              transition: "all 0.2s ease",
              padding: "4px 8px",
              borderRadius: "4px",
              backgroundColor: visibleLines[
                item.dataKey as keyof typeof visibleLines
              ]
                ? "rgba(0, 0, 0, 0.05)"
                : "rgba(0, 0, 0, 0.02)",
              textDecoration: visibleLines[
                item.dataKey as keyof typeof visibleLines
              ]
                ? "none"
                : "line-through",
